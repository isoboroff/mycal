...
pub fn vbyte_encode(value: u32, buffer: &mut Vec<u8>) -> usize {
    let mut written = 0;
    let mut write_byte = (value & 0x7f) as u8;
    let mut value = value >> 7;
    while value > 0 {
        write_byte = write_byte | 0x80;
        buffer.push(write_byte);
        written += 1;
...
pub fn vbyte_decode(buffer: &mut Vec<u8>) -> (u32, usize) {
...
pub fn encode_magic(docgap: u32, freq: u32, buffer: &mut Vec<u8>) -> usize {
    let mut magic_value;
    let mut bytes = 0;
    if freq < MAGIC_F {
...
// See Algorithm 2
pub fn decode_magic(buffer: &mut Vec<u8>) -> (u32, u32) {
